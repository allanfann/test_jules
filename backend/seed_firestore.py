...
# --- Path setup to import from the 'example' directory ---
# Add the project root to the Python path to allow importing from 'example'
...
try:
    ...
    print("Error: Could not import TREES_DATA from example/example_decision_trees.py", file=sys.stderr)
    print("Please ensure the file exists and the project is run from its root directory.", file=sys.stderr)
    sys.exit(1)
    ...
    for tree_id, tree_data in TREES_DATA.items():
        ...
        if tree_doc_ref.get().exists:
            if overwrite:
                ...
                # The nodes are in a subcollection, so we delete that subcollection
        ...
        # Create the main tree document (without the 'nodes' part)
        # The new data structure has 'tree_id' inside the dict, so we use the whole dict
        # except for the 'nodes' key.
...
if __name__ == "__main__":
    ...
    print("This script will populate your Firestore database with data from example_decision_trees.py.")
    print("--------------------------------------------------------------------------------")
    if args.overwrite:
