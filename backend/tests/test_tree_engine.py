...
class TestDecisionTreeEngine(unittest.TestCase):
    ...
    def test_get_start_node_success(self, mock_get_db):
        ...
        mock_root_node = MagicMock(id='root', exists=True, to_dict=lambda: {
            "type": "DECISION", "text": "Is this a test?", "children": []
        })
        ...
        mock_nodes_collection = MagicMock(document=lambda doc_id: mock_root_ref if doc_id == 'root' else MagicMock(get=MagicMock(exists=False)))
    ...
    def test_get_next_node_success(self, mock_get_db):
        ...
        mock_root_node = MagicMock(id='root', exists=True, to_dict=lambda: {
            "type": "DECISION", "text": "Is this a test?",
            "children": [{"answer_text": "Yes", "next_node_id": "outcome_yes"}]
        })
        mock_yes_node = MagicMock(id='outcome_yes', exists=True, to_dict=lambda: {"type": "OUTCOME", "text": "Success!"})
    ...
    def test_get_next_node_from_outcome_node(self, mock_get_db):
        ...
        mock_outcome_node = MagicMock(id='outcome', exists=True, to_dict=lambda: {"type": "OUTCOME"})
